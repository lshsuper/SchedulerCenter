...
namespace SchedulerCenter.Host.Service
{
    public class JobService
    {
        ...
        public JobService(QuartzProvider provider, Utility.Dapper.DapperProvider dapperProvider) {
            _provider = provider;
        ...
        public async Task<ApiResult<bool>> AddJob(TaskOptions opt)
        {
            ...
            try
            {
                ...
                var res =await _provider.AddCronJob<HttpJob>(new AddCronJobOPT {
                    ...
                    JobData=new Dictionary<string, object>() { 
                       ...
                       [Constant.HttpJobDetailKey.ApiUrl]=opt.ApiUrl,
                       [Constant.HttpJobDetailKey.ApiMethod]=opt.RequestType,
                       [Constant.HttpJobDetailKey.ApiAuthKey] =opt.AuthKey,
                       [Constant.HttpJobDetailKey.ApiAuthValue] = opt.AuthValue,
        ...
        /// <returns></returns>
        public async Task<ApiResult<bool>> UpdateJob(TaskOptions opt)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public  async Task<List<TaskOptions>> GetJobs()
        {
            ...
            try
            {
                ...
                groups.ForEach(async(groupName, i)=> {
                    ...
                    jobs.ForEach(async(jobKey,j) => {
                        ...
                        list.Add(new TaskOptions()
                        {
                            GroupName = jobKey.Group,
                            TaskName = jobKey.Name,
                            Status = await _provider.GetTriggerState(trigger.Key.Group, trigger.Key.Name),
                            Describe = job.Description,
                            Interval = (trigger as CronTriggerImpl)?.CronExpressionString,
                            LastRunTime = trigger.GetPreviousFireTimeUtc()?.LocalDateTime,
                            ApiUrl = job.JobDataMap.GetString(Constant.HttpJobDetailKey.ApiUrl),
                            RequestType = job.JobDataMap.GetString(Constant.HttpJobDetailKey.ApiMethod)
                        });
        ...
        public async Task<IEnumerable<TaskLog>> GetJobLogPage(string taskName, string groupName, int page, int pageSize = 5) {
           ...
           var data = await _dapperProvider.QueryAsync<LogModel>(dataSql, new { job_name = taskName, job_group = groupName, offset=offset, limit=pageSize });
            ...
            return data.Select((s) => new TaskLog {
                 Msg=s.Content,
